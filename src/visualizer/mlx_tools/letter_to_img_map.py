...
class LetterToImageMapper:
    ...
    def create_map(self) -> None:
        ...
        """
        w = ImgInfo.WORD_W  # Horizontal spacing between each letter
        h = ImgInfo.WORD_H  # Vertical spacing between each letter
        try:
            # all this parameters depends on the xpm image file
            # and need manual selection at first
            self.extract_different_letter_types(
                self.cap, ImgInfo.CROP_W, ImgInfo.CROP_H, ImgInfo.X_OFF,
                ImgInfo.Y_OFF, w, h, 0)
            self.extract_different_letter_types(
                self.small, ImgInfo.CROP_W, ImgInfo.CROP_H, ImgInfo.X_OFF,
                ImgInfo.Y_OFF, w, h, 264)
            self.extract_different_letter_types(
                self.num, ImgInfo.CROP_W, ImgInfo.CROP_H, ImgInfo.X_OFF,
                ImgInfo.Y_OFF, w, h, 528)
            self.extract_different_letter_types(
                self.symbols, ImgInfo.CROP_W, ImgInfo.CROP_H, ImgInfo.X_OFF,
                ImgInfo.Y_OFF, w, h, 712)
